...
__global__ std::enable_if_t<(width > 0) && fastllm::_typeConvert<scalar_t>::exists> fused_add_rms_norm_kernel(
    ...
    static_assert(std::is_pod_v<fastllm::_f16Vec<scalar_t, width>>);
    static_assert(sizeof(fastllm::_f16Vec<scalar_t, width>) == sizeof(scalar_t) * width);
    ...
    auto *__restrict__ input_v = reinterpret_cast<fastllm::_f16Vec<scalar_t, width> *>(input);
    auto *__restrict__ residual_v = reinterpret_cast<fastllm::_f16Vec<scalar_t, width> *>(residual);
    auto *__restrict__ weight_v = reinterpret_cast<const fastllm::_f16Vec<scalar_t, width> *>(weight);

    for (int idx = threadIdx.x; idx < vec_hidden_size; idx += blockDim.x) {
        int id = blockIdx.x * vec_hidden_size + idx;
        int64_t strided_id = blockIdx.x * vec_input_stride + idx;
        fastllm::_f16Vec<scalar_t, width> temp = input_v[strided_id];
        temp += residual_v[id];
    ...
    for (int idx = threadIdx.x; idx < vec_hidden_size; idx += blockDim.x) {
        int id = blockIdx.x * vec_hidden_size + idx;
        int64_t strided_id = blockIdx.x * vec_input_stride + idx;
        fastllm::_f16Vec<scalar_t, width> temp = residual_v[id];
        temp *= s_variance;
        temp *= weight_v[idx];
...
#define LAUNCH_FUSED_ADD_RMS_NORM(width)                                                                               \
    FASTLLM_DISPATCH_FLOAT_TYPES(input.dataType, {                                                                    \
        LAUNCH_KERNEL(fused_add_rms_norm_kernel<scalar_t, width><<<grid, block>>>((scalar_t *)cudaInput, input_stride, \
            (scalar_t *)cudaResidual, (scalar_t *)cudaWeight, epsilon, num_tokens, hidden_size));                      \
    });
...
bool fused_add_rms_norm( // [hidden_size]
    ...
    int num_dims = input.dims.size();
    axis = (axis % num_dims + num_dims) % num_dims;
    int64_t input_stride = input.strides[axis];
    ...
    FastllmCudaFinishOutput(input, cudaInput);
    FastllmCudaFinishOutput(residual, cudaResidual);
    FastllmCudaFinishInput(weight, cudaWeight);
